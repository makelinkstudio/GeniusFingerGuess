...
import com.bncq.model.MyCommentAdapter;
...
public class MyCommentFragment extends Fragment {
	...
	//UI
	private PullToRefreshListView lvw_myComment;
	...
	public View onCreateView(LayoutInflater inflater,ViewGroup viewGroup,Bundle savedInstanceState) {
		...
		isInit=true;
		//UI
		View myComLayout= inflater.inflate(R.layout.mycomment_layout, viewGroup, false);
		lvw_myComment=(PullToRefreshListView) myComLayout.findViewById(R.id.lvw_myComment);
		return myComLayout;
	}
	@Override
	...
	private void loadData() {
		if (isInit) {  
	         isInit = false;//加载数据完成  
			initListView();
			getLvHandler=UIUtil.getLvHandler(lvw_myComment, myCommentAdapter, lvw_foot_more, lvw_foot_progress, MainApplication.PAGE_SIZE,adapterList);
			loadLVWData(1, getLvHandler, UIUtil.LISTVIEW_ACTION_SCROLL);
		}
	}
	//初始化我的话题列表
	private void initListView(){
		myCommentAdapter=new MyCommentAdapter(getActivity(), adapterList);
	    ...
	    lvw_myComment.addFooterView(lvw_footer);
	    lvw_myComment.setAdapter(myCommentAdapter);
	    lvw_myComment.setTag(UIUtil.LISTVIEW_DATA_MORE);
		// item点击事件
	    lvw_myComment.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			@Override
	    ...
	    lvw_myComment.setOnScrollListener(new AbsListView.OnScrollListener() {
			@Override
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				lvw_myComment.onScrollStateChanged(view, scrollState);
				//数据为空--不用继续下面代码了
				...
				int lvDataState =Integer.parseInt(lvw_myComment.getTag().toString());
			...
			public void onScroll(AbsListView view, int firstVisibleItem,int visibleItemCount, int totalItemCount) {
				// TODO Auto-generated method stub
				lvw_myComment.onScroll(view, firstVisibleItem, visibleItemCount, totalItemCount);
			}
		});
		//刷新
	    lvw_myComment.setOnRefreshListener(new PullToRefreshListView.OnRefreshListener() {
	...
	private void loadLVWData(final int pageIndex,final Handler handler,final int action){
		new Thread(new Runnable() {
			...
			public void run() {
				...
				List params=CommonUtil.setParams(CommonUtil.initPostMap(APIModel.MY_REPLY_TOPIC, dataMap), CommonUtil.ARRAY_LIST);
				ArrayList<HashMap<String, Object>> retList=topicDao.getMyReplyTopData(params);
