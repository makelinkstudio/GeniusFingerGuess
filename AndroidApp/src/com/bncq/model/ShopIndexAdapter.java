...
import com.bncq.controller.ShopDetailActivity;
...
public class ShopIndexAdapter extends BaseAdapter {
	...
	private static class ViewHolder{
		...
		TextView tvw_shop_endTime;
		TextView tvw_shop_appoint;
		TextView tvw_shop_collect;
		ProgressBar pbr_shop_probar;
	}
	public ShopIndexAdapter(Activity activity,ArrayList<HashMap<String, Object>> data) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		ViewHolder holder;
		if(convertView==null){
			convertView=layoutInflater.inflate(R.layout.shop_index_listitem, null);
			...
			holder.tvw_shop_endTime=(TextView) convertView.findViewById(R.id.tvw_shop_endTime);
		...
		holder.tvw_shop_name.setText(listData.get(position).get("BUSIN_NAME").toString()+"【提供】");
		holder.tvw_shop_businName.setText(listData.get(position).get("AVD_TITLE").toString());
	    ...
	    holder.tvw_shop_endTime.setText(MainApplication.getContext().getResources().getString(R.string.shop_endTime)+DateUtil.UnixTimeToDate(listData.get(position).get("AVD_ETIME").toString()));
		//异步加载图片
		...
		//点击跳转事件
		holder.ivw_shop_goods.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				String avd_id=listData.get(position).get("AVD_ID").toString();
				if(StringUtil.stringiSNotEmpty(avd_id)){
					Intent intent =new Intent(activity,ShopDetailActivity.class);
					intent.putExtra("shop_avdId", avd_id);
	...
	private class appointClickListener implements OnClickListener{
		...
		public void onClick(View v) {
			...
			if(appointString.equals(MainApplication.getContext().getResources().getString(R.string.shop_appoint))){//预约状态
				UIUtil.showAlertDialog("确认预约？", new positiveClick(), null,activity);
			}else{
